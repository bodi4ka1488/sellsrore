...
const Modal = ({ active, setActive }) => {
  ...
  const setVisiblePassword = () => {
    setEyeVal(!eyeVal);
    eyeVal
      ? setEye("https://cdn-icons-png.flaticon.com/512/7615/7615155.png")
      : setEye("https://cdn-icons-png.flaticon.com/512/159/159604.png");
      ...
      >
        <form onClick={(e) => preventHandler(e)}>
          ...
          {surnameDirty && surnameError && (
            <div className={style.error}>{surnameError}</div>
          )}
          <input
            value={surname}
          ...
          <button onClick={login} className="google-button">
            <img
              src="https://www.pngplay.com/wp-content/uploads/12/Google-PNG-Photos.png"
              height="25px"
              width="25px"
              alt="GoogleImage"
            ></img>{" "}
          ...
          <button
            onClick={() => {
              setUserData(name, surname, password, true);
              setActive(false);
            }}
            className="submit"
            disabled={!formValid}
